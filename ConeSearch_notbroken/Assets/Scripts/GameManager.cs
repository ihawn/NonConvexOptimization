...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            float x = UnityEngine.Random.Range(-2*bounds, 2*bounds);
            float z = UnityEngine.Random.Range(-2*bounds, 2*bounds);
            float y = Objective(x, z);
            Vector3 point = new Vector3(x, y, z);
            dg.GraphPoint(gameObject, point, 0.01f, Color.black, isFunc: true);
        }
    ...
    {
        return Objectives.Parabolic(x, z);
    }
}
